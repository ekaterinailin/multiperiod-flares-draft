...
%-------------------------------------------------------------------
%\begin{table*}
%\caption{Qualitative detectability assessment for extreme combinations of inclination $i$ and flare latitude $\theta_f$.}
%\begin{tabular}{lcr}
%\hline\hline
%  & high $i$ (equator-on view) & low $i$ (pole-on view)\\
%\hline
%high $\theta_f$ (polar flare) & bad visibility, varying modulation & good visibility, low modulation \\
%low $\theta_f$ (equatorial flare) & good visibility, clear modulation & bad visibility, varying modulation \\
%\hline
%\end{tabular}
%\end{table*}
...
237880881_21_deg_latitude_flare_compilation_scatter_2.png}
    \caption{Flares with the same FWHM, and $a=1$, injected at diffent phases of the light curve and at different latitudes onto a star with $i=21$ deg. Red: Amplitude of the detected primary peak. Magenta: Amplitude of the detected secondary peak.}
...
\begin{figure}
    ...
    \caption{Flares with the same FWHM, and $a=1$, injected at diffent phases of the light curve and at different latitudes onto a star with $i=87$ deg. Red: Amplitude of the detected primary peak. Magenta: Amplitude of the detected secondary peak.}
...
However, given the small number of detections, it could also be a selection effect. If flares on FCUFRs occur at all latitudes equally likely, it could just be that detectability of multiperiod flares is lower for low-latitude flares. Assuming that $i$ is uniformly distributed we find low detectability can stem from flaring at the limb, and therefore being less visible in general, or absence of rotational modulation of the star. We would only find periodically modulated flares on stars without rotational variability if they were modulated, whereas we can identify multi-period flares that are both modulated and not modulated on stars that show rotational variability. In Figs. \ref{fig:discuss_low_lat} and \ref{fig:discuss_high_lat} we demonstrate how the same underlying flare appears in a light curve for a star that is seen at low and high inclination, respectively. We injected flares at different phases of the stellar rotation, that is, starting, for instance, behind the limb or facing the observer, and measured their primary and secondary peaks. 

%\end{itemize}
%at the equator at low inclinations if the primary peak is less pronounced, and low detectability at high inclinations if the secondary peak is confused for an independent event because the the secondary peak is separated by a full half period from the primary peak. On the other hand, low detectability at the pole is found at high inclination because the primary peak height is geometrically reduced, and at low inclination because the secondary peak is less pronounced because the geometrical modulation declines.
\\
Another explanation could be that flares close to the equator occur on active regions with shapes that are not modulated by rotation, as, for instance, active belt shapes at fixed latitudes. 
